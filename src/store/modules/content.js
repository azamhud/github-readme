import {SET_USERNAME, SET_REPOS, SET_FOUND, SET_SEARCHING, SET_README, SET_SELECT, SET_TOAST_TEXT} from '../modules/mutation-types';
import axios from 'axios'
import { resolve } from 'url';
import { CHECK_USERNAME, GET_REPOS } from './action-types';
...
const actions = {
    ...
    async GET_REPOS({dispatch, commit, state}){
        await axios.get(`https://api.github.com/users/${state.username}/repos`)
                                .then((response)=>{
    ...
    async GET_README({dispatch,commit,state}, id){
        const repos = state.repos;
        const repo = repos.find((repo)=> repo.id === id);
        const url = `https://raw.githubusercontent.com/${state.username}/${repo.name}/master/README.md`;
